...
void logRace(raceData_t *race, uint8_t type) {
  ...
  if(type==0) {
    ...
    adjustedTargetTime=race->legTargetTime();
    adjustedDriveSpeed=race->legAdjustedTargetSpeed(mph);
    ...
    actualDistanceComplete=race->legDistanceComplete(mph);
    distanceComplete=actualDistanceComplete;
    distanceRemaining=race->legDistanceRemaining(mph);
    eolDistanceRemaining=distanceRemaining;
    time=race->legTime();
    targetTimeComplete=TIME_INTERNAL_TO_SECONDS(race->activeLeg->targetTime);
    timeComplete=time;
    averageSpeed=race->legAverageSpeed(mph);
    adjustedAverageSpeed=race->legAdjustedAverageSpeed(mph);
    speedDelta=race->legSpeedDelta(mph);
    eolSpeedDelta=speedDelta;
    timeDelta=race->legTimeDelta();
    ...
    targetDistanceComplete=race->raceTargetDistanceComplete(mph);
    driveDistanceComplete=race->raceDriveDistanceComplete(mph);
    actualDistanceComplete=race->raceActualDistanceComplete(mph);
    distanceComplete=race->raceDistanceComplete(mph);
    distanceRemaining=race->raceDistanceRemaining(mph);
    eolDistanceRemaining=race->raceLegEndDistanceRemaining(mph);
    time=race->raceTime();
    targetTimeComplete=race->raceTargetTimeComplete();
    timeComplete=race->raceTimeComplete();
    averageSpeed=race->raceAverageSpeed(mph);
    adjustedAverageSpeed=race->raceAdjustedAverageSpeed(mph);
    speedDelta=race->raceSpeedDelta(mph);
    eolSpeedDelta=race->raceLegEndSpeedDelta(mph);
    timeDelta=race->raceTimeDelta();
    startTs=0;
    endTs=0;
  }
